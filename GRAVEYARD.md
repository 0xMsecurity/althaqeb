...
- **"Broad neutral authority in AI security" as the IDENTITY** (positioning review,
  ...
  there solo is not defensible for a single maintainer. **Replacement (winner):** the *narrow*
...
- **"Academic-first" as the IDENTITY** (same review) — killed as identity, **retained as a
  move**. A paper decays and *commoditizes the method*, eroding the scarcity that is part of the
  moat (an open academic cross-engine benchmark would let anyone replicate `vdbresidue`). It does
  not hold up as the position. A preprint remains a useful authority-conferring
  *action* sequenced AFTER the tool/standard/disclosure — never the thing Althaqeb *is*.
- **Disclosure framing as "CVE-track / critical vulnerability"** (severity-framing review,
